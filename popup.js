...
document.addEventListener('DOMContentLoaded', function () {
  ...
  startCrawlBtn.addEventListener('click', function () {
    try {
        ...
        try {
          ...
          chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
            try {
              ...
              console.error('[闲鱼采集] 标签页处理失败:', error);
          ...
          console.error('[闲鱼采集] 配置读取失败:', error);
          showToast('启动爬取失败: ' + error.message, 'error');
          // 恢复按钮状态
          isCrawling = false;
          startCrawlBtn.textContent = '🚀 开始爬取';
      ...
      console.error('[闲鱼采集] 爬取启动失败:', error);
      showToast('启动爬取失败: ' + error.message, 'error');
      // 恢复按钮状态
      isCrawling = false;
      startCrawlBtn.textContent = '🚀 开始爬取';
  ...
  sendFeishuBtn.addEventListener('click', async function () {
    ...
    chrome.runtime.sendMessage({ type: 'GET_STATS' }, (statsResponse) => {
      ...
      chrome.runtime.sendMessage({ type: 'GET_FEISHU_CONFIG' }, (feishuResponse) => {
        ...
        // 当启用自动创建表格时，只需要 spreadsheetToken，否则需要 productTableId
        if (!config.spreadsheetToken) {
          showToast('请先配置飞书表格 Token', 'warning');
          return;
        }
        if (!config.autoCreateTable && !config.productTableId) {
          showToast('请先配置飞书商品表 ID（或启用「自动创建表格」功能）', 'warning');
          return;
        }
  ...
  function displaySuggestWords(words) {
    ...
    // 清空显示区域（使用 replaceChildren 替代 innerHTML，符合安全规范）
    suggestWordsList.replaceChildren();
    suggestWordsHidden.replaceChildren();
